...
export const generatePage = (pageName, { skipClient, skipTests, skipImport }) => {
  ...
  fs.writeFileSync(`${pageFolder}${pageName}.scss`, '/* import your global stylesheets here */')
  ...
  if (!skipImport && !skipClient) {
    ...
    updateServerIndex(lowerName, pageName)
  }
}
...
const updateServerIndex = (name, snakeName) => {
  const exportRegex = /export default [\s\S]*/g
  const filePath = './app/server/index.ts'
  read(filePath, (data) => {
    const [exportExpression] = data.match(exportRegex)
    const content = `import ${name}Route from '../pages/${snakeName}';\n${data}`
      .replace(exportExpression, `router.use('/${snakeName}', ${name}Route);\n${exportExpression}`)
